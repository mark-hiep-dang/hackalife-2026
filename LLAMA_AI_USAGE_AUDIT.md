...
- No model routing, no task typing, no structured-output validation beyond a shallow `matchesShape()` type-check, no generation cache, no usage logging, no explicit `DEMO_MODE` flag (the app has been running entirely on its deterministic fallbacks all session because the configured Gemini key has zero quota — so "demo mode" was already the de facto behavior, just not modeled as a first-class switch).
...
- **Main model** (`GEMINI_MAIN_MODEL`, default `gemini-2.0-flash`): `GENERATE_CURRICULUM`, `GENERATE_LESSON_KIT` (defined, currently unused per §2.1), `GENERATE_COMPLEX_SCENARIO` (defined, not yet wired to a call site), `GENERATE_RESCUE_EXPEDITION`, `SUGGEST_COMPLEX_QUALITY_FIX`, `TRAINER_COPILOT_COMPLEX` (defined, not yet wired).
- **Light model** (`GEMINI_LIGHT_MODEL`, default `gemini-2.0-flash-lite`): `REWRITE_QUESTION`, `GENERATE_FLASHCARDS` (defined, not yet wired — flashcards are deterministic per §2.1), `SUMMARIZE_INSIGHT`, `EXPLAIN_METRIC`, `TRAINER_COPILOT_SIMPLE`, `LEARNER_CHAT`.
...
**On the specific model names**: the requesting spec asked for defaults of `gemini-3.5-flash` / `gemini-3.1-flash-lite`. I could not confirm these are real, currently-servable Gemini model IDs (this repo's own working integration uses `gemini-2.0-flash`, and I have no way to verify newer names against the live API since the configured key has zero quota). I used `gemini-2.0-flash` / `gemini-2.0-flash-lite` as the defaults instead — safe, known-real names — and left `GEMINI_MAIN_MODEL`/`GEMINI_LIGHT_MODEL` fully overridable via env so you can point at `gemini-3.5-flash` etc. the moment you've confirmed it's a valid model for your key. If a configured name turns out invalid, the call fails and the existing deterministic fallback takes over automatically — it will not break the app, just silently stay on fallback until the env var is corrected.
...
## 9. Tests added (`backend/tests/aiUsageAudit.test.js`, 34 tests)
...
Full suite: **110/110 passing** (76 pre-existing + 34 new), zero regressions. Frontend production build unaffected (no frontend files touched this pass).
...
- Could not verify any of this against a **real, quota-available Gemini call** — the configured key has returned `RESOURCE_EXHAUSTED` all session. Every code path above was verified via its deterministic-fallback branch (which is what the app actually runs on right now) plus the demo/cache logging paths, not via a live model response.
